...
enum class DateFilterOptions(private val title: String, val beginPeriod: LocalDate, val finishPeriod: LocalDate) :
    OptionFilterable {
    //TODO нужно уточнить как начинается неделя в целевой системе возможно с воскресенья
    CURRENT_WEEK(
        "Current week", LocalDate.now().with(DayOfWeek.MONDAY), LocalDate.now().with(DayOfWeek.SUNDAY)
    ),
    PREVIOUS_WEEK(
        "Previous week",
    ...
    CURRENT_MONTH(
        "Current month", LocalDate.now().with(firstDayOfMonth()), LocalDate.now().with(lastDayOfMonth())
    ),
    PREVIOUS_MONTH(
        "Previous month",
